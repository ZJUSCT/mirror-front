...
// image ("Rendering error: Concurrent server side render limit reached") whose
// size never matches the requested width/height. Checking the IHDR dimensions
// keeps that placeholder out of the published output.
export function pngDimensions(image) {
  ...
  for (const panel of config.panels) {
    ...
    for (const theme of ['light', 'dark']) {
      ...
      if (dimensions.width !== config.width || dimensions.height !== config.height) {
        throw new Error(
          `Rendering ${panel.id}/${theme} returned ${dimensions.width}x${dimensions.height} instead of ${config.width}x${config.height}`
        );
      }
...
// Serves the rendered files so mirror-front NGINX can proxy /statistics-data/
// here. Requests only ever need GET and HEAD.
export function startFileServer(output, port) {
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      response.writeHead(200, {
        'Content-Type': CONTENT_TYPES[target.slice(target.lastIndexOf('.'))] ?? 'application/octet-stream',
...
async function main() {
  ...
  const server = values.once ? null : startFileServer(values.output, port);
