...
export function linkWithI18n<P extends { to: string, onClick?: React.MouseEventHandler }>(Component: React.ComponentType<P>) {
  ...
  return ({ to, language, onClick, ...rest }: P & { language?: string }) => {
    ...
    const getLanguagePath = (language: string) => {
      return i18n.generateDefaultLanguagePage || language !== i18n.defaultLanguage
        ? `/${language}`
        : '';
    };
