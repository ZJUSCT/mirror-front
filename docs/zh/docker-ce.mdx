...
export const genCommand = (os) => {
    ...
    #添加软件仓库:\n
    echo \\\n
      "deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/docker.gpg] https://mirror.zju.edu.cn/docker-ce/linux/${o} \\\n
...
```bash
sudo sed -i 's+download.docker.com+mirror.zju.edu.cn/docker-ce+' /etc/yum.repos.d/docker-ce.repo
```
