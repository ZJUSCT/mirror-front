...
- 浙江大学镜像： https://mirror.zju.edu.cn/CTAN/
...
Linux 下使用如下命令：
```bash
perl install-tl --repository https://mirror.zju.edu.cn/CTAN/systems/texlive/tlnet
```
...
```bash
tlmgr option repository https://mirror.zju.edu.cn/CTAN/systems/texlive/tlnet
```
...
```bash
tlmgr update --all --repository https://mirror.zju.edu.cn/CTAN/systems/texlive/tlnet
```
...
```bash
mpm --set-repository=https://mirror.zju.edu.cn/CTAN/systems/win32/miktex/tm/packages/
```