...
cpan[2]> o conf urllist unshift https://mirror.zju.edu.cn/CPAN/

# 或将本站镜像加入镜像列表末尾
# 注：本命令和上面的命令执行一个即可，修改列表不会执行自动去重
cpan[3]> o conf urllist push https://mirror.zju.edu.cn/CPAN/

# 或清空镜像列表，仅保留本站
cpan[4]> o conf urllist https://mirror.zju.edu.cn/CPAN/
...
if ! (
    ...
    grep -qF 'https://mirror.zju.edu.cn/CPAN/'
); then
    perl -MCPAN -e 'CPAN::HandleConfig->load();' \
        -e 'CPAN::HandleConfig->edit("urllist", "unshift", "https://mirror.zju.edu.cn/CPAN/");' \
